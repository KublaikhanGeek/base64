...
int base64_encode(unsigned char *in, unsigned char *out, int out_len)
{
    ...
    for (i = 0; i < nloop; i += 3)
    {
        out[n] = data_bin2ascii[in[i] >> 2];                                    //取出第一个字符的前6位并找出对应的结果字符
        out[n + 1] = data_bin2ascii[(in[i] & 0x3) << 4 | (in[i + 1] >> 4)];     //将第一个字符的后位与第二个字符的前4位进行组合并找到对应的结果字符
        out[n + 2] = data_bin2ascii[(in[i + 1] & 0xf) << 2 | (in[i + 2] >> 6)]; //将第二个字符的后4位与第三个字符的前2位组合并找出对应的结果字符
        out[n + 3] = data_bin2ascii[in[i + 2] & 0x3f];                          //取出第三个字符的后6位并找出结果字符
        n += 4;
    }
